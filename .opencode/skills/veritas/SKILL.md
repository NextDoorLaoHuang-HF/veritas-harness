...
curl -s -X POST "https://YUANDIAN_API_BASE_PLACEHOLDER/open/rh_fg_search" \
...
真实端点：`https://YUANDIAN_API_BASE_PLACEHOLDER`。**不是** `https://open.chineselaw.com`。
错误地址会返回 HTML/JSON 错配，401 报"key 无效"是**伪装**——实际是路由不通或鉴权头错位。
CSP 响应头会泄露真实域名，可作为校验手段。
